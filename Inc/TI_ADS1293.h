...
#define ADS1293_CSB_PIN  GPIO_PIN_4				//PC4
#define ADS1293_DRDY_PIN GPIO_PIN_4				//PA4
...
void Ads1293_Init(void);	//static void spi_Init(uint8_t sck = -1, uint8_t miso = -1, uint8_t mosi = -1, uint8_t ss = -1);
void Ads1293_Config(bool* is_init, SPI_HandleTypeDef hspi1);
void Test_Mode(bool* is_init);
...
void Read_Data_Stream(uint8_t* data, int length, SPI_HandleTypeDef hspi1);		//static void Read_Data_Stream(char* data, int length = 6);
